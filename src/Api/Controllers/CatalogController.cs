...
public class CatalogController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<ActionResult<List<StoreBannerDto>>> GetStoreBanners(Guid id, CancellationToken ct) =>
        Ok(await mediator.Send(new GetStoreBannersQuery(id), ct));
    ...
    public async Task<ActionResult<PagedResult<StoreSectionDto>>> GetStoreSections(
        ...
        Ok(await mediator.Send(new GetStoreSectionsQuery(id, page, pageSize), ct));

    // Matches api_reference.md exactly: status/minPrice/maxPrice/page. The
    // previous version called the query with (id, page, pageSize), which
    // doesn't match its actual parameter order and drops every filter.
